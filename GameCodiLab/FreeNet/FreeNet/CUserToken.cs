...
namespace FreeNet
{
    public class CUserToken
    {
        ...
        /// <summary>
        /// 이 매소드에서 직접 바이트 데이터를 해석해도 되지만
        /// message resolver클래스를 따로 둔 이유는
        /// 추후에 확장성을 고려하여 다른 resolver를 구현할 때 CUserToken클래스의 코드 수정을 최소화
        /// 하기 위함입니다.
        /// </summary>
        ...
        public void send(CPacket msg)
        {
            lock (this.cs_sending_queue)
            {
                ...
                // 큐에 무언가 들어있다면 아직 이전 전송이 완료되지 않은 상태이므로 
                //큐에 추가만 하고 리턴
                // 현재 수행중인 SendAsync가 완료된 이후에 큐를 검사하여 데이터가 있으면
                // SendAsync를 호출하여 전송해 줄 것이다.
                this.sending_queue.Enqueue(msg);
            }
        }

        // 닷넷의 비동기 매소드를 호출하는 부분을 살펴보겠습니다.
        /// <summary>
        /// 비동기 전송을 시작
        /// </summary>
        void start_send()
        {
            lock (this.cs_sending_queue)
            {
                ...
                // 패킷 내용을 SocketAsyncEventArgs 버퍼에 복사합니다.
