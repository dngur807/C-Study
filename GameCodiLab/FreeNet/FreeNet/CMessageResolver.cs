...
namespace FreeNet
{
    ...
    class CMessageResolver
    {
        ...
        // 소켓 버퍼로부터 데이터를 수신할 때 마다 호출됩니다.
        // 데이터가 남아 있을 때 까지 게속 패킷을 만들어 callback을 호출 준다.
        // 하나의 패킷을 완성하지 못했다면 버퍼에 보관해 놓은 뒤 다음 수신을 기다린다.

        public void on_receive(byte[] buffer, int offset, int transfferd, CompletedMessageCallback callback)
        {
            // 이번 receive로 읽어오게 될 바이트 수
            this.remain_bytes = transfferd;

            // 원본 버퍼의 포지션값
            // 패킷이 여러개 뭉쳐 올 경우 원본 버퍼의 포지션은 계속 앞으로
            // 가야 하는데 그처리를 위한 변수
            ...
            while (this.remain_bytes > 0)
            {
                ...
                {
                    // 목표 지검 설정 (헤더 위치까지 도달하도록 설정)
                    this.position_to_read = Defines.HEADERSIZE;

                    completed = read_until(buffer, ref src_position, offset, transfferd);

                    if (!completed)
                    {
                        // 아직 다 못읽었으므로 다음 receive를 기다립니다.
                        return;
                    }
                    // 헤더 하나를 온전히 읽어왔으면 메시지 사이즈를 구한다.
            ...
            bool read_until(byte[] buffer, ref int src_position, int offset, int transffered)
            {
                ...
                // 읽어 와야 할 바이트
                // 데이터가 분리되어 올 경우 이전에 읽어 놓은 값을 빼줘서 
                // 부족한 만큼 읽어올 수 있도록 계산해 준다.
        ...
        {
            // 헤더에서 메시지 사이즈를 구한다.
            // 헤더 타입은 Int16 Int32 두가지가 올 수 있으므로 각각을 구분하여 처리한다.
            // 사ㅣ실 헤더가 바뀔 경우는 없을 테니 한가지로 고정하는 편이 깔끔할것 같다.
